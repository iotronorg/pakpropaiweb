  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute text-center">
        <p className="text-xl font-bold text-[var(--text-primary)] tabular-nums">{rate}%</p>
        <p className="text-[10px] text-[var(--text-muted)] leading-none">success</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-7 pb-10">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">AI Performance Monitor</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="rounded-xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-6 flex flex-col items-center gap-4">
          ...
          <div className="w-full grid grid-cols-2 gap-3 text-center text-xs">
            <div className="rounded-lg bg-[var(--bg-surface)]/70 py-2.5">
              <p className="text-lg font-bold text-[var(--text-primary)] tabular-nums">{s.leads_with_convos}</p>
              <p className="text-[var(--text-muted)]">Total Conversations</p>
            </div>
            <div className="rounded-lg bg-[var(--bg-surface)]/70 py-2.5">
              <p className="text-lg font-bold text-[var(--text-primary)] tabular-nums">{s.qualified_leads}</p>
              <p className="text-[var(--text-muted)]">Qualified</p>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-5">
          <h3 className="text-sm font-semibold text-[var(--text-muted)] mb-5">AI Routing Queue State</h3>
              ...
              return (
                <div key={label}>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-xs font-bold text-[var(--text-primary)] tabular-nums">{Math.max(value, 0)}</span>
                  </div>
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-5 flex flex-col gap-5">
          <div>
            <p className="text-xs text-[var(--text-muted)] mb-1">New Leads This Week</p>
            <p className="text-4xl font-bold text-[var(--text-primary)] tabular-nums">{s.new_this_week}</p>
          </div>
          {Object.keys(channelBreakdown).length > 0 && (
            <div>
              <p className="text-xs font-semibold text-[var(--text-muted)] mb-3">Conversation Channels (recent)</p>
          ...
          <div className="mt-auto pt-3 border-t border-[var(--border)]">
            <div className="grid grid-cols-2 gap-2 text-center text-xs">
              <div>
                <p className="text-lg font-bold text-[var(--text-primary)] tabular-nums">{s.hot_leads}</p>
                <p className="text-[var(--text-muted)]">Hot Today</p>
              </div>
              <div>
                <p className="text-lg font-bold text-[var(--text-primary)] tabular-nums">{s.routing_queue}</p>
                <p className="text-[var(--text-muted)]">Queue Depth</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)]">
        <div className="border-b border-[var(--border)] px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-[var(--text-primary)]">Recent AI Conversations</h2>
            <p className="text-xs text-[var(--text-muted)] mt-0.5">Last 15 inbound messages handled by the AI</p>
          </div>
          <span className="text-xs text-[var(--text-muted)]">{convos.length} shown</span>
        </div>
        {convos.length === 0 ? (
          <p className="text-center py-12 text-sm text-[var(--text-muted)]">No conversations yet</p>
        ) : (
              ...
              <div key={c.lead_id + c.created_at} className="grid grid-cols-[auto_1fr_auto] items-start gap-4 px-6 py-3.5">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-medium text-[var(--text-primary)]">{c.lead_name ?? c.lead_phone}</p>
                    {c.lead_name && (
                      <span className="text-xs text-[var(--text-muted)]">{c.lead_phone}</span>
                    )}
                  </div>
                  <p className="mt-0.5 text-sm text-[var(--text-muted)] line-clamp-2">{c.message_preview}</p>
              ...
              <thead>
                <tr className="border-b border-[var(--border)]">
                  {["Agent", "City", "Leads", "Qualified", "Deals", "Rating", "Status"].map((h) => (
                    <th key={h} className="px-6 py-3 text-start text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {topAgents.map((a) => (
                  <tr key={a.id} className="hover:bg-[var(--bg-muted)]/50 transition-colors">
                    ...
                    <td className="px-6 py-3 font-bold text-[var(--text-primary)] tabular-nums">{a.total_leads}</td>
                    <td className="px-6 py-3 text-[var(--text-muted)] tabular-nums">{a.closed_leads}</td>
                    <td className="px-6 py-3 text-[var(--text-muted)] tabular-nums">{a.closed_deals}</td>
                    <td className="px-6 py-3">
                      {a.rating > 0 ? (
                        <span className="font-medium text-[var(--text-primary)]">⭐ {a.rating.toFixed(1)}</span>
                      ) : (
