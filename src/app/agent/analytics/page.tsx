...
import { ClipboardList, Flame, CheckCircle2, Home, Lock, Zap, TrendingUp } from "lucide-react";
...
export default function AgentAnalyticsPage() {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">My Analytics</h1>
        <p className="mt-1 text-sm text-[var(--text-muted)]">Your personal performance metrics</p>
      </div>

      {/* KPI row */}
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-7 gap-4">
        <StatCard label="Total Leads"    value={stats.total_leads   ?? 0} accent="blue"    icon={ClipboardList} />
        <StatCard label="Hot Leads"      value={stats.hot_leads     ?? 0} accent="amber"   icon={Flame} sub={`Avg score ${stats.avg_score ?? 0}`} />
        <StatCard label="Closed Leads"   value={stats.closed_leads  ?? 0} accent="emerald" icon={CheckCircle2} />
        <StatCard label="My Listings"    value={stats.total_listings ?? 0} accent="violet"  icon={Home} />
        <StatCard label="Closed Deals"   value={stats.closed_deals  ?? 0} accent="rose"    icon={Lock} />
        <StatCard
          label="Response Time"
        ...
        />
        <StatCard label="Conversion"     value={`${convRate}%`}           accent="blue"    icon={TrendingUp} sub={`Rating ${stats.rating?.toFixed(1) ?? "—"}`} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] p-5">
          ...
          {Object.keys(stats.by_source ?? {}).length > 0 && (
            <div className="mt-4 pt-4 border-t border-[var(--border)]">
              <p className="text-xs font-medium text-[var(--text-muted)] mb-2">By Source</p>
        ...
        <ChartCard title="Lead Volume" period={period} onPeriodChange={setPeriod}>
          ...
          <div className="mt-4 pt-4 border-t border-[var(--border)] grid grid-cols-3 gap-3 text-center">
            <div>
              <p className="text-lg font-bold text-[var(--text-primary)] tabular-nums">{stats.total_leads ?? 0}</p>
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)]">Hot</p>
            </div>
          </div>
        </ChartCard>
      </div>
