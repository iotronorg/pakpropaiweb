...
function ScoreBar({ score }: { score: number | null }) {
  if (score === null) return <span className="text-xs text-[var(--text-muted)]">Not scored</span>;
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs text-[var(--text-muted)] w-12 text-right">{score}/100</span>
    </div>
  );
}
    ...
    <div>
      ...
      {expanded && images.length > 0 && (
        <div className="mt-2 grid grid-cols-3 gap-1.5">
          {images.map((img) => (
            <div key={img.id} className="relative group rounded-md overflow-hidden bg-[var(--bg-subtle)] h-20">
...
function EditPropertyModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-xl rounded-xl bg-[var(--bg-surface)] shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-[var(--border)] px-6 py-4">
          <h3 className="font-semibold text-[var(--text-primary)]">Edit Listing</h3>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">City <span className="text-red-500">*</span></label>
              <input className={inputCls} value={city} onChange={(e) => setCity(e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Location / Area</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Area</label>
              <input type="number" className={inputCls} value={areaMarla} onChange={(e) => setAreaMarla(e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Construction Status</label>
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Furnished Status</label>
...
function DealLockInitiateModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-xl bg-[var(--bg-surface)] shadow-xl">
        <div className="flex items-center justify-between border-b border-[var(--border)] px-6 py-4">
          <h3 className="font-semibold text-[var(--text-primary)]">Lock Deal</h3>
          <button type="button" onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-secondary)] text-lg leading-none">×</button>
        </div>
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Token Amount <span className="text-red-500">*</span></label>
            <input
            ...
            />
            <p className="text-xs text-[var(--text-muted)] mt-1">25,000 – 100,000</p>
          </div>
          <div>
            <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1">Payment Gateway</label>
          ...
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
              filter === f
                ...
                : "bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
            }`}
          >
            {f.charAt(0).toUpperCase() + f.slice(1)}
            <span className={`ms-1.5 ${filter === f ? "text-blue-200" : "text-[var(--text-muted)]"}`}>
              {counts[f]}
            </span>
          </button>
        ))}
          ...
          {properties.map((p) => (
            ...
            >
              ...
              <div>
                ...
                {p.ref_no && (
                  <p className="mt-0.5 text-xs font-mono text-[var(--text-muted)]">{p.ref_no}</p>
                )}
                <p className="mt-0.5 text-sm text-[var(--text-muted)]">{p.location}, {p.city}</p>
              </div>
              ...
              <div className="flex items-baseline justify-between">
                ...
                {p.area_sqm && (
                  <span className="text-sm text-[var(--text-muted)]">{formatArea(p.area_sqm)}</span>
                )}
              </div>
              ...
              {/* Primary image */}
              {p.primary_image && (
                <div className="rounded-lg overflow-hidden bg-[var(--bg-subtle)] h-32">
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-[var(--bg-surface)] rounded-xl p-6 w-full max-w-sm shadow-xl">
            <h3 className="font-semibold text-[var(--text-primary)] mb-2">Delete Listing?</h3>
